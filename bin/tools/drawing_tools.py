#!/usr/bin/python
"""
The DrawingTools module receives images and draws on them

Input: Image and Objects to draw
Output: Image
...
class DrawingTools(object):
    ...
    def get_cnn_metadata(self, mode):
        rospy.wait_for_service(mode + '_bridge/get_cnn_metadata')
        try:
            get_metadata = None
            if mode == 'segmentation':
                get_metadata = rospy.ServiceProxy(
                    'segmentation_bridge' + '/get_cnn_metadata', getSegmentationMetadata)
            elif mode == 'detection':
                get_metadata = rospy.ServiceProxy(
                    mode + '_bridge/get_cnn_metadata', getDetectionMetadata)
